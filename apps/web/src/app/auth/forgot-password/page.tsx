...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setLoading(true);
    setError('');
    const response = await fetchApi<{ message: string }>('/auth/forgot-password', {
      method: 'POST',
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-neutral-950 px-4 py-12 text-neutral-100 selection:bg-amber-400 selection:text-neutral-950 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl border border-neutral-800 bg-neutral-900/80 p-8 sm:p-10 backdrop-blur-2xl shadow-2xl">
        ...
        {message && (
          <div className="mb-6 flex items-center gap-2 rounded-2xl border border-emerald-900/60 bg-emerald-950/40 p-3.5 text-xs text-emerald-300">
        ...
        <p className="mt-6 text-center text-xs text-neutral-400">
          ...
          <Link className="text-amber-300 font-semibold hover:underline" href="/auth/login">
            Return to sign in
          </Link>
        </p>
      </div>
